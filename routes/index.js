...
//     let myReport = "";
//     if (req.params.id === "1") {
//         myReport = "Report for week 1. Frontend"
    ...
    jwt.verify(token, process.env.JWT_SECRET, function(err, decoded) {
        if (err) {
            ...
            if (report) {
                ...
                console.log("Du är inloggad. Din text har sparats i databasen.");
                console.log(report);
                const sqlite3 = require('sqlite3').verbose();
                const db = new sqlite3.Database('./db/texts.sqlite');

                db.run("INSERT INTO reports (report) VALUES (?)", report, (err) => {
                    if (err) {
                        // returnera error
                        console.log(err);
                    }
                ...
                console.log("Tomma strängar sparas inte i databasen.");
            }
        }
    });
